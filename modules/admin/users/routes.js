...
app.all(modelPath + "/add", isUserLogedIn, async (req, res) => {
  if(isPost(req)){
    ...
    .withMessage(res.__("admin.user.phone_should_be_min_to_max_characters_long",MIN_PHONE_NUMBER_LIMIT, MAX_PHONE_NUMBER_LIMIT))
    .custom((value, { req }) => {
    ...
    .not()
    .isEmpty()
    .withMessage(res.__("admin.user.please_enter_user_name"))
    .isLength({ min: MIN_NAME_LIMIT, max: MAX_NAME_LIMIT })
    .withMessage(res.__("admin.user.username_should_be_min_to_max_characters_long",MIN_NAME_LIMIT, MAX_NAME_LIMIT))
    .custom((value, { req }) => {
      let collection = db.collection("users");
      return collection.findOne({ "user_name": value }).then((userDoc) => {
        if (userDoc) {
          return Promise.reject(res.__("admin.user.user_name_is_already_in_use_please_try_something_different"));
        }
      });
    })
    .run(req);
    ...
    getDropdownList(req, res, options).then(response=>{
      ...
      res.render('add',{
        country  : (response.final_html_data[0]) ? response.final_html_data[0] : '',
        interest : (response.final_html_data[1]) ? response.final_html_data[1] : '',
      });
    })
  }
});
